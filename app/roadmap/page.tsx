...
const displayBadge = (project: { health: string; status: string }) => {
  const status = project.status.trim().toLowerCase();
  if (status === 'on hold') return { label: 'On hold', className: 'watch' };
  if (status === 'closed') return { label: 'Closed', className: 'closed' };
  return { label: project.health, className: healthClass[project.health] };
};
