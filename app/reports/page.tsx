...
export default function ReportsPage() {
  ...
  const { applySnapshot, snapshotDate, snapshots, restoreSnapshot, deleteSnapshot } = useDashboardData();
  ...
  const removeSnapshot = (date: string) => { if (window.confirm(`Delete the ${date} local snapshot? This does not remove the SharePoint files.`)) { deleteSnapshot(date); setMessage(`Deleted the ${date} local snapshot. You can import a corrected file pair to recreate it.`); } };
  return <main className="with-sidebar reports-page"><Sidebar /><div className="page-content">
    ...
    <section className="upload-grid"><label><span>1. Full project snapshot</span><strong>{projectFile?.name ?? 'Choose project snapshot (.xlsx)'}</strong><input ref={projectInput} type="file" accept=".xlsx" onChange={event => setProjectFile(event.target.files?.[0] ?? null)} /><button type="button" onClick={() => projectInput.current?.click()}>Choose project file</button></label><label><span>2. Incremental Flash reports</span><strong>{flashFile?.name ?? 'Choose Flash reports (.xlsx)'}</strong><input ref={flashInput} type="file" accept=".xlsx" onChange={event => setFlashFile(event.target.files?.[0] ?? null)} /><button type="button" onClick={() => flashInput.current?.click()}>Choose Flash file</button></label></section>
    ...
    <p className="reports-note"><strong>Import rule:</strong> file names do not matter—the dashboard checks the spreadsheet columns. The project file replaces the current local project snapshot. Flash reports add only new reports; existing Flash history is retained and duplicate project/date combinations are ignored.</p>
    ...
    <section className="snapshot-archive"><div><p className="eyebrow">LOCAL SNAPSHOT ARCHIVE</p><h2>Retained project snapshots</h2><p>Restore an older snapshot at any time. To correct a recent snapshot, delete it here, then import the corrected file pair to recreate it.</p></div><div>{[...snapshots].reverse().map(snapshot => <article key={snapshot.date}><div><strong>{snapshot.date}</strong><small>{snapshot.projects.length} projects · {snapshot.importedAt === 'Baseline' ? 'Original baseline' : `Imported ${snapshot.importedAt}`}</small></div><div className="snapshot-actions"><button type="button" disabled={snapshot.date === snapshotDate} onClick={() => restoreSnapshot(snapshot.date)}>{snapshot.date === snapshotDate ? 'Current' : 'Restore'}</button>{snapshot.importedAt !== 'Baseline' && <button type="button" className="delete-snapshot" onClick={() => removeSnapshot(snapshot.date)}>Delete</button>}</div></article>)}</div></section>
  </div></main>;
}
